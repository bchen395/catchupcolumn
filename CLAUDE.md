...
6. **Email delivery** — When an Edition publishes, send an email to all Group members (via Resend). The email is a first-class design surface: full content including photos (signed URLs, 1-year TTL), brand-styled newspaper layout, content-led subject, and https links to `WEB_BASE_URL` (`https://www.catchupcolumn.com` — `www`, never the apex). See `supabase/functions/_shared/edition-email.ts` and the `edge-functions` skill.
